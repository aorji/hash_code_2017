...
struct DataCenter
{
    ...
    void Output(void)
    {
        ...
        for(int i = 0; i < caches.size(); ++i)
        {
            ...
            {
                std::cout << "video idx: ";
                for(auto & idx: caches[i].video_idx)
                    std::cout << idx << " ";
                std::cout << std::endl;
            }
        }
    ...
    void Arrange(void)
    {
        ...
        {
            while(!cache.endpoint_latency.empty()) //till we have ep conected to the particular cache
            {
                ...
                for(int i = 0; i < ep->video_request.size(); ++i)   //go though all videos starting from the most profitamle
                {
                    if (ep->closed[i])  //if video was already cached, continue
                        continue;
                    if (cache.capacity >= ep->video_request[i].size)    //if cache has enough space for particular video
                    {
